...
function read_lxc_template(){
    last_lxc_version=$(curl -Ls "https://api.github.com/repos/hzgz/lxcde/releases/latest" | grep "LXC" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [[ -n $last_lxc_version ]]; then
        image_list=$(curl -Ls "https://api.github.com/repos/hzgz/lxcde/releases/latest" | grep "LXC" | grep '"browser_download_url":' | sed -E 's/.*"([^"]+)".*/\1/')

        os_list=$(curl -Ls "https://api.github.com/repos/hzgz/lxcde/releases/latest" | grep "LXC" | grep '"browser_download_url":' | sed -E 's/.*"([^"]+)".*/\1/' | sed "s/https\:\/\/github.com\/hzgz\/lxcde\/releases\/download\/${last_lxc_version}\///g" | sed "s/\.tar\.gz//g")
        echo "$os_list" | nl
...
function read_openvz_template(){
    releasetag="v0.0.1"
    os_list=$(wget -qO- "https://github.com/hzgz/lxcde/releases/expanded_assets/v0.0.1" | \
    ...
    os_selected=$( echo "$os_list" | head -n $os_index | tail -n 1)
    download_link="https://github.com/hzgz/lxcde/releases/download/${releasetag}/${os_selected}.tar.gz"
}
